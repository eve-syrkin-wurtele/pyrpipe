...
This framework minimizes the commands user has to write, because the tools are available as objects and are fully re-usable.
All commands executed via pyrpipe are automatically logged and can be compiled into extensive reports using the pyrpipe_diagnostic tool.

To facilitate processing of RNA-Seq data, we have implemented specialized classes build on top of the pyrpipe framework.
...
pyrpipe can be used on local computers or on HPC environments; pyrpipe scripts can be easily integrated into workflow management systems such as Snakemake or Nextflow.
...
- Automatically resume pipelines/jobs if they are interrupted
- Easily integrated into workflow managers like Snakemake or NextFlow
